    ...
    if df is not None:
        ...
        try:
            ...
            if not selected_features:
                ...
                # Add clustering buffer option
                st.subheader("Clustering Size Constraints (Optional)")
                use_constraints = st.checkbox("Apply size constraints to clusters")
                
                if use_constraints:
                    clustering_buffer = st.slider(
                        "Buffer percentage (±)",
                        min_value=0,
                        max_value=50,
                        value=15,
                        help="Controls how much clusters can vary in size. A buffer of 15% means clusters can be 15% larger or smaller than the average size."
                    ) / 100.0
                    
                    n_samples = len(X_scaled)
                    avg_cluster_size = n_samples / k
                    
                    size_min = max(1, int(round(avg_cluster_size * (1 - clustering_buffer))))
                    size_max = int(round(avg_cluster_size * (1 + clustering_buffer)))
                    
                    st.info(f"""
                    With {n_samples} samples divided into {k} clusters:
                    - Average cluster size: {avg_cluster_size:.1f} samples
                    - Min cluster size: {size_min} samples
                    - Max cluster size: {size_max} samples
                    """)
        ...
        2. **Clustering Analysis**:
           ...
           - Option to apply size constraints to clusters (min/max size)
